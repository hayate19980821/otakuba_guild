window.GuildStorage = (() => {
  ...
  function normalizeMonster(m, i){
    m = m || {};
    const hpMax = Number(m.maxHp || m.hp || 500) || 500;
    return {
      id:m.id || GuildUtils.uid('enemy'),
      name:m.name || `敵${i+1}`,
      stage:m.stage || '草原',
      hp:Number.isFinite(Number(m.hp)) ? Math.max(0,Number(m.hp)) : hpMax,
      maxHp:hpMax,
      bg:m.bg || m.background || 'grass.png',
      image:m.image || 'slime.png',
      bgm:m.bgm || 'slime'
    };
  }
  ...
  function migrateLegacy(legacy){
    ...
    return {
      ...
      menu: Array.isArray(legacy.products) ? legacy.products.map(p => ({
        id:p.id||GuildUtils.uid('menu'),
        cat:p.cat||p.category||'food',
        category:p.cat||p.category||'food',
        name:p.name||'商品',
        price:Number(p.price)||0,
        emoji:p.emoji||p.icon||'🍽️',
  ...
  async function init(){
    ...
    if(typeof data.settings.coverCharge === 'undefined') data.settings.coverCharge = 500;
  ...
  function save(){
    ...
    set(keys.menu, data.menu);
    set(keys.monsters, data.monsters);
    set(keys.customers, data.customers);
    set(keys.sales, data.sales);
  }
