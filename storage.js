window.GuildStorage = (() => {
  ...
  let data = {settings:{}, menu:[], monsters:[], customers:[], sales:[], currentCustomer:'', partyCount:1, activeBill:[], currentEnemyIndex:0};
  ...
  function normalizePartyCount(v){ return Math.max(1, Math.min(20, Number(v)||1)); }
  function migrateLegacy(legacy){
    ...
    return {
      ...
      partyCount: normalizePartyCount(legacy.partyCount || legacy.party || 1),
  ...
  async function init(){
    const defaults = {
      ...
      currentCustomer:'', partyCount:1, activeBill:[], currentEnemyIndex:0
    };
    data = defaults;
    ...
    data.partyCount = normalizePartyCount(data.partyCount);
  ...
  function setPartyCount(count){ data.partyCount = normalizePartyCount(count); save(); return data.partyCount; }
  ...
  return {keys, init, save, getData, replace, resetProgress, setPartyCount};
})();
