window.GuildOrder = (() => {
  ...
  function itemFromProduct(p, qty=1){
    ...
    return {id:p.id, name:p.name, cat:p.cat||p.category||'food', price, qty:Number(qty)||1, subtotal:price*(Number(qty)||1)};
  }
  ...
  function addToBill(item){
    ...
    const old=data.activeBill.find(x=>x.id===item.id && !x.isCharge);
  ...
  function record(type, items, total, reason=''){
    ...
    const rec={id:GuildUtils.uid(type==='checkout'?'sale':'order'), type, customer:c.name, items, total, partyCount:data.partyCount||1, time:new Date().toISOString(), timeText:todayText(), reason};
  ...
  function payload(type, sale, items, total, extra={}){
    ...
    return Object.assign({
      ...
      sale,
      adventurerId:c&&c.id||'',
      adventurer:c&&c.name||sale.customer||'未登録',
      name:c&&c.name||sale.customer||'未登録',
      title:c&&c.title||'',
      level:c&&c.level||1,
      visits:c&&c.visits||0,
      ...
      items,
      total,
      enemy:e?{name:e.name,hp:e.hp,maxHp:e.maxHp,defeated:false}:null,
      source:'index',
      appVersion:GuildApp.VERSION,
      time:todayText()
    },extra);
  }
