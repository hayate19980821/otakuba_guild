window.GuildUI = (() => {
  ...
  function applyBg(bg){ const page=$('pageBg'), battle=$('battleField'); const u = (window.GuildUtils&&GuildUtils.driveImg?GuildUtils.driveImg(bg):bg) || 'background.jpg'; const pageBg=`linear-gradient(to bottom,rgba(0,0,0,.08),rgba(0,0,0,.20) 55%,rgba(0,0,0,.75)),url("${u}")`; const battleBg=`linear-gradient(to bottom,rgba(0,0,0,.04),rgba(0,0,0,.32)),url("${u}")`; if(page) page.style.backgroundImage=pageBg; if(battle) battle.style.backgroundImage=battleBg; }
